...
Backend-сервис веб-игры на **Kotlin + Spring Boot 3**. Приложение работает с PostgreSQL через хранимые функции и процедуры (`user_*`, `task_*`, `suggestion_*`, `achievements_*`).
...
## API: Предложения задач (`/suggestion`)

Предложения новых задач от игроков (модерация). Игрок отправляет идею задачи, администратор может просмотреть список и удалить ненужные.

---

### `GET /suggestion/get` — все предложения

**Параметры:** нет.

**Успех — `200 OK`**, тело: `Result<List<Suggestion>>`:
...
| Поле | Тип | Описание |
|------|-----|----------|
| `id` | long | Id предложения |
| `header` | string | Заголовок |
| `description` | string | Описание задачи |
| `answer` | string | Предлагаемый ответ |
| `points` | int | Предлагаемые баллы |
| `hint` | string \| null | Подсказка |
| `authorId` | long | Id автора (`user_id`) |

> Список может быть пустым `[]`, если предложений ещё нет.

---

### `POST /suggestion/create` — создать предложение
...
| `answer` | string | да | Предлагаемый ответ |
| `points` | int | да | Баллы |
| `hint` | string \| null | нет | Подсказка |
| `authorId` | long | да | Id автора (должен существовать в БД) |

**Успех — `200 OK`**, тело: `Result<Long>` — id нового предложения.

**Ошибки:** необработанные исключения БД (например, если пользователь-автор не найден) вернут `500`.

---

### `DELETE /suggestion/delete` — удалить предложение
...
Передаётся **число** — id предложения (Long).

**Успех — `200 OK`**, тело: `Result<Unit>`.

**Ошибки:** если предложение не найдено, PostgreSQL вернёт «Предложение не найдено» → `SuggestionNotFoundException`.
